...
from src.core.scheduler import (
    ScheduledJobManager, ScheduledJobSpec, CronParser, ExecutionRecord,
    ScheduleStatus, SchedulerProtocol
)
from src.core.reconciler import JobType, JobState
...
class TestScheduledJobManager:
    ...
    @pytest.mark.asyncio
    async def test_remove_scheduled_job_success(self, job_manager, sample_scheduled_job):
    ...
    @pytest.mark.asyncio
    async def test_scheduler_execution_check(self, job_manager, sample_scheduled_job, mock_scheduler):
        ...
        assert len(history) >= 0  # Execution may or may not have happened depending on timing
    ...
    @pytest.mark.asyncio
    async def test_get_scheduler_statistics_with_jobs(self, job_manager, sample_scheduled_job):
    ...
    async def test_execution_history_limit(self, job_manager):
        ...
        if len(job_manager._execution_history[job_id]) > 100:
            job_manager._execution_history[job_id] = job_manager._execution_history[job_id][-100:]
