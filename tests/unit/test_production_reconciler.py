...
from src.core.reconciler_fixed import (
    ProductionJobReconciler, ReconcilerConfig, ReconciliationResult,
    ReconciliationStatistics, FlinkClientProtocol, StateStoreProtocol,
    ChangeTrackerProtocol, MetricsCollectorProtocol, CircuitBreakerProtocol
)
...
from src.core.exceptions import (
    ...
    ReconciliationError, JobDeploymentError, SavepointError, FlinkClusterError,
    ErrorCode
)
...
class MockCircuitBreaker:
    ...
    def __init__(self):
        self.call = Mock(side_effect=lambda func, *args, **kwargs: func(*args, **kwargs))
        self.is_open = False


class TestProductionJobReconciler:
    """Comprehensive tests for ProductionJobReconciler."""
    ...
    def reconciler(self, mock_flink_client, mock_state_store, mock_change_tracker,
        ...
        return ProductionJobReconciler(
    ...
    def minimal_reconciler(self, mock_flink_client):
        ...
        return ProductionJobReconciler(flink_client=mock_flink_client)
    ...
    @pytest.mark.asyncio
    async def test_reconcile_job_update_batch_job(self, reconciler, sample_batch_spec,
                                                 mock_flink_client, mock_change_tracker):
    ...
    @pytest.mark.asyncio
    async def test_reconcile_job_redeploy_stopped_job(self, reconciler, sample_streaming_spec,
                                                     mock_flink_client):
    ...
    @pytest.mark.asyncio
    async def test_reconcile_job_concurrent_reconciliation(self, reconciler, sample_streaming_spec, mock_flink_client):
    ...
    @pytest.mark.asyncio
    async def test_reconcile_job_concurrent_timeout_cleanup(self, reconciler, sample_streaming_spec):
    ...
    @pytest.mark.asyncio
    async def test_reconcile_job_circuit_breaker_open(self, reconciler, sample_streaming_spec,
                                                     mock_circuit_breaker, mock_flink_client):
        """Test handling circuit breaker open."""
        mock_circuit_breaker.is_open = True
        mock_flink_client.get_cluster_info.side_effect = Exception("Circuit breaker open")
    ...
    @pytest.mark.asyncio
    async def test_reconcile_job_flink_cluster_error(self, reconciler, sample_streaming_spec,
                                                    mock_flink_client):
        ...
        mock_flink_client.get_job_details.side_effect = FlinkClusterError("Cluster unreachable")
    ...
    @pytest.mark.asyncio
    async def test_reconcile_job_deployment_error(self, reconciler, sample_streaming_spec,
                                                 mock_flink_client):
    ...
    @pytest.mark.asyncio
    async def test_reconcile_job_savepoint_error(self, reconciler, sample_streaming_spec,
                                                mock_flink_client, mock_change_tracker):
    ...
    @pytest.mark.asyncio
    async def test_reconcile_job_unexpected_exception(self, reconciler, sample_streaming_spec,
                                                     mock_flink_client):
        """Test handling unexpected exceptions."""
        mock_flink_client.get_cluster_info.side_effect = RuntimeError("Unexpected error")
    ...
    @pytest.mark.asyncio
    async def test_reconcile_all_single_job(self, reconciler, sample_streaming_spec, mock_flink_client):
    ...
    @pytest.mark.asyncio
    async def test_reconcile_all_multiple_jobs_concurrent(self, reconciler, mock_flink_client):
    ...
    @pytest.mark.asyncio
    async def test_reconcile_all_mixed_success_failure(self, reconciler, mock_flink_client):
        """Test reconciling with mixed success and failure results."""
        specs = [
            JobSpec(job_id="success-job", job_type=JobType.STREAMING, artifact_path="/success.jar"),
            JobSpec(job_id="failure-job", job_type=JobType.STREAMING, artifact_path="/failure.jar")
        ]
    ...
    @pytest.mark.asyncio
    async def test_reconcile_all_exception_conversion(self, reconciler, mock_flink_client):
        """Test that exceptions in concurrent processing are converted to failed results."""
        specs = [
            JobSpec(job_id="test-job", job_type=JobType.STREAMING, artifact_path="/test.jar")
        ]
        
        # Mock to raise unexpected exception
        mock_flink_client.get_cluster_info.side_effect = RuntimeError("Unexpected runtime error")
    ...
    @pytest.mark.asyncio
    async def test_state_store_integration(self, reconciler, sample_streaming_spec,
    ...
    @pytest.mark.asyncio
    async def test_change_tracker_integration(self, reconciler, sample_streaming_spec,
    ...
    @pytest.mark.asyncio
    async def test_metrics_integration(self, reconciler, sample_streaming_spec,
    ...
    @pytest.mark.asyncio
    async def test_metrics_error_recording(self, reconciler, sample_streaming_spec,
                                          mock_metrics_collector, mock_flink_client):
        """Test error metrics recording."""
        # Trigger an error during reconciliation
        mock_flink_client.get_job_details.side_effect = Exception("Job details fetch failed")
    ...
    def test_statistics_update(self, reconciler):
        ...
        assert abs(stats.average_duration_ms - 116.67) < 0.01  # (100 + 200 + 50) / 3, with tolerance
    ...
    def test_state_conversion_edge_cases(self, reconciler):
        ...
        assert reconciler._convert_flink_state_to_job_state("RUNNING") == JobState.RUNNING
        assert reconciler._convert_flink_state_to_job_state("FINISHED") == JobState.STOPPED
        assert reconciler._convert_flink_state_to_job_state("CANCELED") == JobState.STOPPED
        assert reconciler._convert_flink_state_to_job_state("CANCELLED") == JobState.STOPPED
        assert reconciler._convert_flink_state_to_job_state("FAILED") == JobState.FAILED
        assert reconciler._convert_flink_state_to_job_state("RESTARTING") == JobState.RECONCILING
        
        # Test unknown state
        assert reconciler._convert_flink_state_to_job_state("UNKNOWN_STATE") == JobState.UNKNOWN
    ...
    async def test_circuit_breaker_call_wrapper(self, reconciler, mock_circuit_breaker):
        ...
        result = await reconciler._call_with_circuit_breaker(mock_async_func, "test1", "test2")
        assert result == "result: test1, test2"
    
    @pytest.mark.asyncio
    async def test_minimal_reconciler_without_optional_components(self, minimal_reconciler, sample_streaming_spec):
        ...
        minimal_reconciler._flink_client.get_job_details.side_effect = Exception("Job not found")
