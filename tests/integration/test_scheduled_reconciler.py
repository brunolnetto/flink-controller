...
from src.core.reconciler import ScheduledJobReconciler, JobSpec, JobType, JobState
from src.core.scheduler import ScheduledJobManager, ScheduledJobSpec
from src.core.types import safe_cast_job_id, safe_cast_cron_expression
...
class TestScheduledJobReconciler:
    ...
    def scheduled_reconciler(self, mock_flink_client):
        ...
        from src.core.reconciler import ReconciliationResult, ReconciliationAction
    ...
    @pytest.mark.asyncio
    async def test_execute_job_with_scheduled_spec(self, scheduled_reconciler, scheduled_job_spec, mock_flink_client):
    ...
    async def test_get_job_status(self, scheduled_reconciler, mock_flink_client):
        ...
        # Setup mock job
        mock_flink_client.jobs["test_job"] = {
            'id': 'flink_123',
            'state': 'RUNNING'
        }
    ...
    @pytest.mark.asyncio
    async def test_add_remove_scheduled_job(self, scheduled_reconciler, scheduled_job_spec):
    ...
    @pytest.mark.asyncio
    async def test_scheduled_execution_flow(self, scheduled_reconciler, scheduled_job_spec, mock_flink_client):
    ...
    @pytest.mark.asyncio
    async def test_error_handling_in_execute_job(self, scheduled_reconciler, scheduled_job_spec):
        """Test error handling in job execution."""
        # Mock reconcile_job to raise an exception
        scheduled_reconciler.reconcile_job = AsyncMock(side_effect=Exception("Test error"))
        
        success = await scheduled_reconciler.execute_job(scheduled_job_spec)
        assert success is False
    
    @pytest.mark.asyncio
    async def test_error_handling_in_get_job_status(self, scheduled_reconciler, mock_flink_client):
        """Test error handling in get job status."""
        job_id = safe_cast_job_id("error_job")
        
        # Mock client method to raise exception
        mock_flink_client.get_job_id_by_name = AsyncMock(side_effect=Exception("Connection error"))
