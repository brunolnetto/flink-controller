...
class FlinkCredentials(BaseModel):
    ...
    expires_at: Optional[str] = Field(None, description="Credential expiration timestamp")
...
class CredentialManager:
    ...
    def _initialize_secure_store(self) -> dict:
        ...
        return {
            "store_type": "environment_variables",
    ...
    def validate_credentials(self, credentials: FlinkCredentials) -> bool:
        ...
        if not credentials.username or not credentials.password or not credentials.api_key:
            return False
