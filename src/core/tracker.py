...
class ChangeRecord(BaseModel):
    ...
    changed_at: str = Field(default_factory=lambda: datetime.now(timezone.utc).isoformat(), description="When change was detected")
...
class JobSpecTracker:
    ...
    def calculate_spec_hash(self, spec: JobSpec) -> str:
        ...
        normalized_dict = {k: v for k, v in spec_dict.items() if k not in exclude_fields}
        
        # Convert enum values to strings for JSON serialization
        if 'job_type' in normalized_dict and hasattr(normalized_dict['job_type'], 'value'):
    ...
    async def _identify_changed_fields(self, job_id: str, new_spec: JobSpec) -> List[str]:
        """
    ...
    async def record_change(self, change: ChangeRecord) -> None:
        ...
        """, (
            change.job_id, change.spec_hash, change.previous_hash,
            change.changed_at, change.change_type, changed_fields_json
        ))
    ...
    async def get_change_history(self, job_id: Optional[str] = None, limit: int = 100) -> List[ChangeRecord]:
        """
    ...
    async def remove_tracked_job(self, job_id: str) -> bool:
        ...
        async with self._lock:
            ...
            await self._connection.execute(
                "DELETE FROM tracked_specs WHERE job_id = ?", 
                (job_id,)
            )
            await self._connection.commit()
    ...
    async def batch_update_tracker(self, specs: List[JobSpec]) -> int:
        ...
        async with self._lock:
            ...
            """, [(job_id, spec_hash, current_time, job_id, current_time) for job_id, spec_hash, current_time in updates])
