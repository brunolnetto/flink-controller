...
from ..security.auth import AuthResult, FlinkAuthManager
...
class DeploymentConfig(BaseModel):
    ...
    program_args: List[str] = Field(
        default_factory=list, description="Program arguments"
    )
    ...
    allow_non_restored_state: bool = Field(
        False, description="Allow non-restored state"
    )
...
class FlinkRESTClient:
    ...
    async def get_cluster_info(self) -> FlinkClusterInfo:
        ...
        try:
            ...
            return FlinkClusterInfo(
                ...
                jobs_running=len(
                    [j for j in jobs.get("jobs", []) if j.get("status") == "RUNNING"]
                ),
            )
    ...
    async def get_job_details(self, job_id: str) -> FlinkJobInfo:
        ...
        try:
            ...
            checkpoint_info = {}
            try:
                checkpoint_response = await self._make_request(
                    "GET", f"/jobs/{job_id}/checkpoints"
                )
            ...
            return FlinkJobInfo(
                ...
                start_time=(
                    datetime.fromtimestamp(
                        response.get("start-time", 0) / 1000, tz=timezone.utc
                    ).isoformat()
                    if response.get("start-time")
                    else None
                ),
                end_time=(
                    datetime.fromtimestamp(
                        response.get("end-time", 0) / 1000, tz=timezone.utc
                    ).isoformat()
                    if response.get("end-time")
                    else None
                ),
                duration=response.get("duration", 0),
                parallelism=response.get("parallelism", 1),
                checkpoint_count=checkpoint_info.get("count", 0),
                last_checkpoint=checkpoint_info.get("latest", {}).get(
                    "completed_timestamp", ""
                ),
            )
        except Exception as e:
    ...
    async def _submit_job(
        self, jar_id: str, config: DeploymentConfig, job_name: Optional[str]
    ) -> str:
        ...
        try:
            payload = {
                ...
                "programArgs": (
                    " ".join(config.program_args) if config.program_args else ""
                ),
    ...
    async def stop_job(
        self, job_id: str, savepoint_path: Optional[str] = None, drain: bool = False
    ) -> Optional[str]:
        """
        Stop a running job.
        ...
        try:
            if savepoint_path or drain:
                ...
                payload = {"targetDirectory": savepoint_path, "drain": drain}
                response = await self._make_request("POST", endpoint, data=payload)
    ...
    async def get_savepoint_status(
