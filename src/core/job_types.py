...
class JobTemplate(BaseModel):
    ...
    parameter_schema: Dict[str, Dict[str, Any]] = Field(
        default_factory=dict, description="JSON schema for parameter validation"
    )
...
class JobDependency(BaseModel):
    ...
    wait_strategy: WaitStrategy = Field(
        default=WaitStrategy.COMPLETION, description="How to wait for dependencies"
    )
    timeout_seconds: Optional[int] = Field(
        None, description="Timeout for dependency wait"
    )

    # Conditional dependencies
    condition: Optional[str] = Field(
        None, description="Condition expression for conditional dependencies"
    )
...
class ScheduledJobSpec(JobSpec):
    ...
    job_type: JobType = Field(
        default=JobType.BATCH, description="Job type (must be batch)"
    )
    ...
    max_runs: Optional[int] = Field(
        None, description="Maximum number of runs (None = unlimited)"
    )
    ttl_seconds: Optional[int] = Field(None, description="Job time-to-live in seconds")
    cleanup_on_completion: bool = Field(
        default=True, description="Clean up job after completion"
    )
    ...
    run_count: int = Field(
        default=0, description="Number of times job has been executed"
    )
...
class PipelineJobSpec(JobSpec):
    ...
    fail_pipeline_on_failure: bool = Field(
        default=True, description="Fail entire pipeline if this job fails"
    )
    pipeline_timeout_seconds: Optional[int] = Field(
        None, description="Timeout for entire pipeline"
    )
...
class MultiArtifactJobSpec(JobSpec):
    ...
    artifact_order: List[int] = Field(
        default_factory=list, description="Order of artifact loading"
    )

    # Artifact-specific configurations
    artifact_configs: Dict[str, Dict[str, Any]] = Field(
        default_factory=dict, description="Per-artifact configurations"
    )
    ...
    def validate_main_artifact_index(cls, v, values):
        ...
        if "artifact_paths" in values:
            ...
            if v >= len(artifact_paths):
                raise ValueError(
                    f"Main artifact index {v} is out of bounds for {len(artifact_paths)} artifacts"
                )
        return v
...
class TemplateJobSpec(BaseModel):
    ...
    parameters: Dict[str, str] = Field(
        ..., description="Parameters used to render template"
    )
...
class JobTypeManager:
    ...
    def create_job_from_template(
        self, template_id: str, job_id: str, parameters: Dict[str, str]
    ) -> TemplateJobSpec:
    ...
    def get_pipeline_execution_order(self, pipeline_id: str) -> List[List[str]]:
        ...
        while remaining_jobs:
            ...
            if not ready_jobs:
                ...
                raise ValueError(
                    f"Circular or unsatisfied dependencies in pipeline {pipeline_id}: {remaining_job_ids}"
                )
