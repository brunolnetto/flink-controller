...
from .exceptions import (CircuitBreakerOpenError,
                         ...
                         ReconciliationError, SavepointCreationError,
                         SavepointError)
from .types import FlinkJobId, JobId, safe_cast_job_id
...
class JobSpec(BaseModel):
    ...
    artifact_signature: Optional[str] = Field(
        None, description="Artifact signature for verification"
    )

    # Deployment configuration
    parallelism: int = Field(default=1, description="Job parallelism")
    checkpoint_interval: Optional[int] = Field(
        None, description="Checkpoint interval in ms"
    )
    ...
    savepoint_trigger_interval: Optional[int] = Field(
        None, description="Savepoint trigger interval"
    )
...
class FlinkClientProtocol(Protocol):
    ...
    async def stop_job(
        self, job_id: str, savepoint_path: Optional[str] = None
    ) -> Optional[str]:
...
class MetricsCollectorProtocol(Protocol):
    ...
    def record_reconciliation(
        self, job_id: JobId, action: str, success: bool, duration_ms: int
    ) -> None:
...
class JobReconciler:
    ...
    async def reconcile_job(self, spec: JobSpec) -> ReconciliationResult:
        ...
        try:
            ...
            try:
                ...
                action = await self._determine_reconciliation_action(
                    spec, current_status
                )

                # Execute reconciliation action
                result = await self._execute_reconciliation_action(
                    spec, current_status, action
                )
        ...
        async with self._reconciliation_lock:
            if job_id in self._active_reconciliations:
                ...
                if timeout_duration > self._config.reconciliation_timeout:
                    ...
                    raise ConcurrentReconciliationError(
    ...
    async def _determine_reconciliation_action(
        self, spec: JobSpec, current_status: Optional[JobState]
            ...
            if self._change_tracker:
                ...
                if has_changed:
                    return (
                        ...
                        else ReconciliationAction.STOP
                    )
    ...
    async def _execute_reconciliation_action(
        ...
        try:
            ...
            raise ReconciliationError(
                f"Failed to execute {action.value}", spec.job_id, cause=e
            )
    ...
    async def _update_streaming_job(self, spec: JobSpec) -> None:
        ...
        try:
            ...
            savepoint_request = await self._flink_client.trigger_savepoint(
                spec.job_id, f"/savepoints/{spec.job_id}"
            )
    ...
    async def _restart_job(self, spec: JobSpec) -> None:
        ...
        try:
            ...
            raise JobDeploymentError(
                f"Failed to restart job {spec.job_id}", spec.job_id, cause=e
            )
    ...
    def _update_statistics(self, results: List[ReconciliationResult]) -> None:
        ...
        if durations:
            ...
            total_ops = (
                self._statistics.successful_reconciliations
                + self._statistics.failed_reconciliations
            )
            if total_ops > 0:
        ...
        for result in results:
            ...
            self._statistics.actions_taken[action] = (
                self._statistics.actions_taken.get(action, 0) + 1
            )
        ...
        try:
            ...
            if hasattr(job_spec, "cron_expression"):
                ...
                base_spec = JobSpec(
                    ...
                    restart_strategy=getattr(
                        job_spec, "restart_strategy", "fixed-delay"
                    ),
