...
from .types import (
    JobId, ArtifactPath, JobSpecDict, ValidationErrorDict,
    DatabaseRecordDict, safe_cast_job_id, safe_cast_artifact_path,
    is_valid_job_id, is_valid_artifact_path, is_valid_memory_string
)
...
class StrictValidationResult(BaseModel):
    ...
    errors: List[ValidationErrorDict] = Field(default_factory=list, description="Validation errors")
    warnings: List[ValidationErrorDict] = Field(default_factory=list, description="Validation warnings")
...
class StrictJobSpecManager:
        ...
        for spec_dict in specs:
            ...
            if validation_result.valid:
                try:
                    ...
                    raise ValueError(f"Failed to create JobSpec from {spec_dict.get('job_id', 'unknown')}: {e}")
            else:
                error_messages = [f"{err['field']}: {err['message']}" for err in validation_result.errors]
                raise ValueError(f"Invalid spec for job {validation_result.job_id}: {error_messages}")
        
        # Update cache with strict typing
        self._spec_cache = {safe_cast_job_id(spec.job_id): spec for spec in validated_specs}
    ...
    async def _load_specs_from_files(self) -> List[JobSpecDict]:
        ...
        if not self._spec_directory.exists():
            raise FileNotFoundError(f"Spec directory {self._spec_directory} does not exist")
        ...
        for spec_file in self._spec_directory.glob("*.yaml"):
            try:
                with open(spec_file, 'r', encoding='utf-8') as f:
                    ...
                    validated_spec = self._validate_spec_dict_structure(spec_data, spec_file.name)
    ...
    def _validate_spec_dict_structure(self, data: dict, filename: str) -> JobSpecDict:
        ...
        if not isinstance(artifact_path, str) or not is_valid_artifact_path(artifact_path):
        ...
        if 'checkpoint_interval' in data:
            ...
            if not isinstance(interval, int) or interval < 1000:
                raise ValueError(f"Invalid checkpoint_interval in {filename}: {interval}")
            spec_dict['checkpoint_interval'] = interval
        
        if 'memory' in data:
            memory = data['memory']
            if not isinstance(memory, str) or not is_valid_memory_string(memory):
                raise ValueError(f"Invalid memory specification in {filename}: {memory}")
    ...
    def _convert_dict_to_jobspec(self, spec_dict: JobSpecDict) -> JobSpec:
        ...
        job_type = JobType.STREAMING if spec_dict['job_type'] == 'streaming' else JobType.BATCH
    ...
    async def validate_spec(self, spec_dict: JobSpecDict) -> StrictValidationResult:
        """
    ...
    async def _validate_business_rules(
        ...
        if spec.parallelism < 1:
            ...
            warnings.append(ValidationErrorDict(
                field='parallelism',
    ...
    async def get_job_status(
        self, 
        job_id: JobId, 
        flink_client: Optional[FlinkStatusProtocol] = None
